...
from model.utils.blob import im_list_to_blob          # FIX #4: use same blob fn as demo.py
from model.faster_rcnn.resnet import resnet
from model.roi_layers import nms                       # FIX #5: NMS was never applied
...
def load_model():
    ...
    # FIX: Set anchor config exactly as demo.py does
...
def _get_image_blob(im):
    """
    FIX #3 & #4: Mirror demo.py's _get_image_blob exactly.
    Keeps BGR order, subtracts cfg.PIXEL_MEANS, builds the multi-scale blob.
    """
    im_orig = im.astype(np.float32, copy=True)
    im_orig -= cfg.PIXEL_MEANS          # pixel-mean subtraction (was missing before)
    ...
    blob = im_list_to_blob(processed_ims)           # correct blob format for the model
...
async def detect_contact(request: dict = Body(...)):
    """
    Input:  {"image": "<base64 or data-uri>", "threshold": 0.5}
    Output: {
        "hand_detections":   [[x1,y1,x2,y2, score, contact_state, hand_side], ...],
        "object_detections": [[x1,y1,x2,y2, score], ...],
        "contact_validated": bool,   # True if any hand is NOT in state "N"
        "contact_state_map": {...},
        "num_hands": int,
        "num_objects": int,
    }
    ...
    try:
        ...
        # ── FIX #6: extract contact-state and hand-side from loss_list ──────────
        contact_vector  = loss_list[0][0]               # shape [1, N, 5]
        offset_vector   = loss_list[1][0].detach()      # shape [1, N, 3]
        lr_vector       = loss_list[2][0].detach()      # shape [1, N, 1]

        _, contact_indices = torch.max(contact_vector, 2)          # [1, N]
        ...
        if cfg.TEST.BBOX_REG:
            ...
            if cfg.TRAIN.BBOX_NORMALIZE_TARGETS_PRECOMPUTED:
                ...
                # class-specific (4 * num_classes columns)
        ...
        pred_boxes /= im_scales[0]          # scale back to original image coords
        ...
        # ── Per-class NMS + build output dicts (FIX #5 & #6) ────────────────────
        hand_dets = []
        obj_dets  = []
        ...
        for cls_idx, label, thresh in class_cfg:
            ...
            # FIX #5: apply NMS (was completely missing before)
            ...
            # Map back to original indices for loss_list accessors
            ...
            for k in range(len(keep)):
                ...
                if label == "hand":
                    # FIX #6: attach contact state and hand side
        ...
        # FIX #7: contact_validated = any hand that is NOT in "No contact" state
